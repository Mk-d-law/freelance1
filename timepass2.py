...
"""
    ...
    full_memo = ""  # This will store the full memo content

    # Stream each chunk
    for chunk in response_stream:
        memo_box['text'] += chunk.text
        full_memo += chunk.text  # Accumulate the full memo
        print(chunk.text)
        yield f"event: partial_final_memo\ndata: {json.dumps(chunk.text)}\n\n"

# At the end, yield the full memo once
    yield f"event: full_memo\ndata: {json.dumps({'text': full_memo})}\n\n"
...
# Send message with stream=True to receive chunks as they are generated
    response = chat_session.send_message(query)  # No streaming
